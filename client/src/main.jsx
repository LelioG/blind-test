...
function TournamentScreen({ tournament, token, onTournament, onLeave, setError }) {
  const audioRef = useRef(null);
  const [busy, setBusy] = useState(false);
  ...
  }, [tournament.currentRoundIndex, tournament.status, question?.preview]);

  async function toggleTournamentAudio() {
    const audio = audioRef.current;
    if (!audio || !question?.preview) return;
    setError("");
    setAudioBlocked(false);
    if (audio.paused) {
      try {
    ...
    return (
      <main className="tournamentLobby">
        <section className="heroPanel lobbyPanel">
          ...
          {!isHost && <div className="waitingPulse"><Loader2 className="spin" size={20} /> En attente de l'hôte</div>}
        </section>
      ...
      <section className="roundPanel">
        ...
        <div className="tournamentPlayLayout">
          <div className="tournamentAudioStage">
            <button className={cls("disc", isAudioPlaying && "discPlaying")} onClick={toggleTournamentAudio} aria-label={isAudioPlaying ? "Mettre le son en pause" : "Lancer le son"}>
              <img className="vinylArt" src="/assets/vinyl-record.png" alt="Vinyle en lecture" />
            </button>
            <audio
              ref={audioRef}
              ...
              onPlay={() => { setIsAudioPlaying(true); setAudioBlocked(false); }}
              onPause={() => setIsAudioPlaying(false)}
              onEnded={() => setIsAudioPlaying(false)}
              onError={() => setError("Cet extrait audio ne peut pas être chargé sur cet appareil.")}
            />
            <button className="tournamentAudioButton" type="button" onClick={toggleTournamentAudio} disabled={!question?.preview}>
              {isAudioPlaying ? <Pause size={19} /> : <Play size={19} />}
              {isAudioPlaying ? "Pause" : audioReady ? "Lancer le son" : "Charger le son"}
            </button>
...
function App() {
  ...
  return (
    <div className="appShell">
      ...
      {tournament && <TournamentScreen tournament={tournament} token={tournamentToken} onTournament={openTournament} onLeave={leaveTournament} setError={setError} />}
      {!tournament && game?.stage === "playing" && <GameScreen game={game} onGame={setGame} setError={setError} />}
