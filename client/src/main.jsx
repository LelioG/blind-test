...
function TournamentScreen({ tournament, token, onTournament, onLeave, setError }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!audio) return;
      audio.currentTime = 0;
      audio.play().catch(() => setError("Clique sur le disque pour autoriser la lecture du son."));
    }, delay);
    return () => {
      ...
      <section className="roundPanel">
        ...
        <div className="tournamentPlayLayout">
          <div className="tournamentAudioStage">
            <button className={cls("disc", "discPlaying")} onClick={() => audioRef.current?.play()} aria-label="Relancer le son">
              <img className="vinylArt" src="/assets/vinyl-record.png" alt="Vinyle en lecture" />
            </button>
            <audio ref={audioRef} src={question?.preview || ""} preload="auto" />
            <p className="hostHint">Le son joue sur cet appareil. Appuie sur le disque pour le relancer.</p>
          </div>
