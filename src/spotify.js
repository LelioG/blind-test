...
function spotifyItemToTrack(item) {
  ...
  const artistNames = (track.artists || [])
    ...
    .filter(Boolean);
  const primaryArtist = artistNames[0] || "Artiste inconnu";
  const featuredArtists = artistNames.slice(1);
  const spotifyArtist = artistNames.join(", ") || primaryArtist;

  return {
    spotifyTitle: track.name,
    spotifyArtist,
    spotifyPrimaryArtist: primaryArtist,
    spotifyFeaturedArtists: featuredArtists,
    spotifyFullName: track.name + " - " + spotifyArtist,
  };
}
