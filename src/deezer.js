...
const { extractDeezerPlaylistId, normalizeAnswer, shuffleArray } = require("./utils");
...
function deezerTrackToCommonTrack(track) {
  const title = track && (track.title || track.title_short);
  ...
  for (const spotifyTrack of shuffled) {
    ...
    try {
      ...
      playableTracks.push(commonTrackToPlayableTrack({
        source: "spotify",
        title: spotifyTrack.spotifyTitle,
        artist: spotifyTrack.spotifyArtist,
