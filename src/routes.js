...
const { spotifyEnabled } = require("./config");
...
function registerApiRoutes(app) {
  ...
  app.post("/api/tournaments/join", (req, res) => {
    try {
      const { room, token } = joinTournament(req.body.code, req.body.name);
      res.json({ ok: true, token, tournament: serializeRoom(room, "player", token) });
    } catch (error) {
      sendApiError(res, error.statusCode || 500, error.message || "Impossible de rejoindre la partie.");
    }
  });
  ...
  app.get("/api/tournaments/:code", (req, res) => {
    try {
      ...
      const { room, role } = getTournament(req.params.code, token);
      res.json({ ok: true, tournament: serializeRoom(room, role, token) });
    } catch (error) {
      sendApiError(res, error.statusCode || 500, error.message || "Impossible de charger la partie.");
    }
  });
  ...
  app.post("/api/tournaments/:code/start", (req, res) => {
    try {
      ...
      const room = startTournament(req.params.code, token);
      res.json({ ok: true, tournament: serializeRoom(room, "host", token) });
    } catch (error) {
      sendApiError(res, error.statusCode || 500, error.message || "Impossible de lancer la partie.");
    }
  });
...
function registerRoutes(app) {
  ...
  app.get("/login", (req, res) => {
    if (!spotifyEnabled) {
      return sendOAuthError(
        res,
        503,
        "Spotify n'est pas configuré sur ce serveur.",
        "Ajoute SPOTIFY_CLIENT_ID et SPOTIFY_CLIENT_SECRET dans le fichier .env."
      );
    }
  ...
  app.get("/callback", async (req, res) => {
    if (!spotifyEnabled) {
      return sendOAuthError(res, 503, "Spotify n'est pas configuré sur ce serveur.");
    }
