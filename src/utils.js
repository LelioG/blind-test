...
function uniqueNames(names) {
  ...
  for (const name of names) {
    ...
    const key = normalizeAnswer(cleanName);
...
function splitFeaturedArtistNames(value) {
  return uniqueNames(String(value || "")
    .split(/\s*(?:,|&|\/|\+|\sx\s|\sX\s|\swith\s|;)\s*/g)
    .map((name) => name.replace(/^et\s+/i, "").trim()));
}

function extractFeaturedArtistsFromText(value) {
  const text = String(value || "");
  const matches = [];
  const regex = /(?:\(|\[|\s)(?:feat\.?|ft\.?|featuring|avec)\s+([^\)\]]+?)(?=\)|\]|\s-\s|$)/gi;
  let match;

  while ((match = regex.exec(text))) {
    matches.push(...splitFeaturedArtistNames(match[1]));
  }

  return uniqueNames(matches);
}

function stripFeaturedCreditsFromTitle(value) {
  return String(value || "")
    .replace(/\s*[\(\[]\s*(?:feat\.?|ft\.?|featuring|avec)\s+[^\)\]]+[\)\]]/gi, "")
    .replace(/\s+(?:feat\.?|ft\.?|featuring|avec)\s+.+$/i, "")
    .trim();
}

function normalizeFeaturedArtists(primaryArtist, artists) {
  const primaryKey = normalizeAnswer(primaryArtist);
  return uniqueNames(artists).filter((artist) => normalizeAnswer(artist) !== primaryKey);
}
